import InitFinalizeDiagram from "@/components/diagrams/InitFinalizeDiagram";
import { useLanguage } from "@/lib/LanguageContext";
import { developerContent } from "@/lib/content/developer";

export default function InitFinalizeFlow() {
...
// Compute the init hash (same logic as the contract)
const initHash = ethers.keccak256(
  ethers.AbiCoder.defaultAbiCoder().encode(
    ...
    [tokenAddress, recipientAddress, amount, vaultProof, nonce]
      ...
      <h3>{c.h3Step2}</h3>
      <pre><code>{`// Submit the init hash (no vault proof in this transaction)
const VAULT_ABI = [
  "function initTransfer(bytes32 initHash) external",
];
const vault = new ethers.Contract(safeAddress, VAULT_ABI, signer);

const initTx = await vault.initTransfer(initHash);
await initTx.wait();
// initHash is now stored in the contract with a 600-second expiry`}</code></pre>

      <h3>{c.h3Step3}</h3>
      <pre><code>{`// Wait for initTransfer transaction to be mined
