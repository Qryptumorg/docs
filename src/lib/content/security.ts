  ...
  en: {
    model: {
      ...
      factorItems: [
        ...
        "OTP chain proof: A one-time vault proof is passed as bytes32 in calldata. The contract verifies it against the current OTP chain head (keccak256(proof) == chainHead), then advances the chain head - making each proof position single-use. Replaying a spent proof reverts with \"Proof already used\".",
      ],
      pTwoFactorConclusion:
        "An attacker who has only the private key cannot move tokens because qToken transfers always revert. An attacker who has only the vault proof cannot call vault functions because onlyOwner blocks any address that is not the owner wallet. And even if an attacker observes a proof in calldata, that proof is already consumed: the OTP chain has advanced and the spent proof will never be accepted again.",
      ...
      pTwoFactor2:
        "The OTP chain and commit-reveal transfer scheme prevent replay attacks through three mechanisms:",
      replayItems: [
        "OTP chain (primary): Each vault proof position is single-use. Once consumed, the chain head advances to the next position. Any attempt to re-use a spent proof reverts immediately - the consumed proof position is gone forever.",
        'Commit-reveal nonce: Each commit hash includes a unique random nonce chosen by the browser, so even identical transfer parameters produce a different hash each time. A second attempt with the same hash reverts with "Commit already used".',
        'Commit expiry: A commit expires 600 seconds (10 minutes) after submission. A reveal attempted after this window reverts with "Commit expired".',
      ],
      ...
      attacks: [
        ...
        ["Replay a spent OTP proof", "Revert", "Chain head has advanced - spent proof rejected"],
    ...
    nonCustodial: {
      ...
      custodyRows: [
        [
          ...
          "PersonalVault contract address (0xQryptSafeA)",
        ...
        [
          "OTP chain head (keccak256 hash)",
          "PersonalVault storage (public)",
          "Only advanced by consuming valid OTP proofs; rechargeable via rechargeChain()",
        ],
      ],
    ...
    postQuantum: {
      ...
      h2HowProtects: "How keccak256 Protects the OTP Chain",
      pHowProtects:
        "QryptSafe V6 uses an OTP (one-time proof) chain instead of a static password hash. The contract stores a chainHead - the keccak256 hash of the next valid proof position. The proof itself is never stored:",
      pPreimage:
        "To forge a valid vault proof, an attacker must find the preimage of the current chainHead - inverting keccak256. This is a preimage attack. For a 256-bit hash, the classical complexity is 2^256 operations. Under Grover's algorithm, that is reduced to 2^128 operations, which remains beyond the capacity of any foreseeable quantum computer. By comparison, 128-bit quantum security is the threshold adopted by NIST's post-quantum cryptography standards.",
      calloutNist:
      ...
      futureRows: [
        ...
        [
          ...
          "Full token access",
          "OTP chain head (keccak256) still blocks vault access - forged key cannot produce the next valid OTP proof",
        ],
        [
          "OTP chain head broken by quantum",
          "N/A",
      ...
      honestItems: [
        ...
        "OTP proof in calldata: Each vault proof position appears in transaction calldata during Qrypt(), unQrypt(), and revealTransfer(). However, in V6 each proof is single-use - by the time it appears on-chain, it is already consumed and the chain head has advanced. Observing historical calldata yields spent proofs that the contract will reject. The chain can be recharged via rechargeChain() at any time.",
        "Chain head-to-proof inversion: The stored chainHead cannot be used to derive the next valid proof without inverting keccak256. Only the legitimate vault owner, who knows the full OTP chain derivation, can produce the next valid proof.",
      ],
      pHonestConclusion:
        "The post-quantum resilience of Qryptum is specifically in the OTP chain head storage (keccak256) and the commit-reveal transfer hash. These provide meaningful defense even in scenarios where ECDSA is compromised. The OTP chain structure adds an additional property: observed calldata proofs are already spent and structurally useless to an attacker.",
    },
    contractVerification: {
      ...
      h3V6Active: "v6: Active (OTP chain · air bags · 67/67 E2E · MIT verified)",
      pV6: "Security upgrade from v5. OTP chain replaces static bytes32 proofHash: each proof is single-use and advances the chain head, making replay attacks structurally impossible. Air bags isolation separates QryptAir funds from the shielded balance. rechargeChain() allows OTP top-up. 67/67 E2E tests pass on Sepolia. Factory and implementation MIT-verified on Etherscan.",
      h3V5Active: "v5: Superseded by v6 (bytes32 proofHash · 51/51 E2E · MIT verified)",
      pV5: "Canonical v5 deployment on Sepolia. bytes32 proofHash replaces string vaultProof. unshieldToRailgun() for QryptShield, redeemVoucher() EIP-712 for QryptAir. 51/51 E2E tests pass. All contracts MIT-verified on Etherscan. Superseded by v6 (OTP chain upgrade).",
      ...
      calloutMainnet:
        "QryptSafe is live on Ethereum mainnet. Factory: 0xE3583f8cA00Edf89A00d9D8c46AE456487a4C56f. Implementation: 0x9E73602079fCbB918D22A7a8b57C2d99F5D701b4. Etherscan verification in progress.",
      ...
      coverageRows: [
        ["QryptSafeV1.test.js", "12", "EIP-1167 clone, Ownable factory, shield/unshield, commit-reveal"],
        ["QryptSafeV2.test.js", "23", "Decimal fix, nonce commit, SafeERC20, non-transferable qToken"],
        ["QryptSafeV3.test.js", "36", "No admin keys, changeVaultProof, ECDSA metaTransfer, isolation"],
        ["QryptSafeV4.test.js", "47", "Custom errors (13), vault metadata, partial unshield, activityCount"],
        ["QryptSafeV5.test.js", "51", "qrypt/unqrypt, QryptAir EIP-712 vouchers, Railgun ZK, 8 groups (T01-T51)"],
      ],
      coverageTotal: ["Total", "169", "All passing"],
      h2E2EV6: "E2E Test Results (Sepolia, v6: 67/67)",
      e2eV6Rows: [
      ...
      h2E2EV5: "E2E Test Results (Sepolia, v5: 51/51)",
      e2eV5Rows: [
        ["T01 - Deploy QryptSafeV5 factory + impl (EIP-1167)", "PASS"],
        ["T02 - Verify EIP-1167 bytecode prefix on factory", "PASS"],
        ["T03 - Confirm impl address stored on factory", "PASS"],
        ["T04 - createVault: Wallet A → Vault A clone", "PASS"],
        ["T05 - createVault: Wallet B → Vault B clone", "PASS"],
        ["T06 - approve + qrypt 2.0 USDC (Wallet A)", "PASS"],
        ["T07 - qToken minted, balanceOf Vault A = 2.0", "PASS"],
        ["T08 - unqrypt 1.0 USDC (partial amount)", "PASS"],
        ["T09 - qToken balance = 1.0 after partial unqrypt", "PASS"],
        ["T10 - qrypt reject: wrong proof → InvalidProof()", "PASS"],
        ["T11 - qrypt reject: below minimum → InvalidAmount()", "PASS"],
        ["T12 - qrypt reject: non-owner → NotOwner()", "PASS"],
        ["T13 - unqrypt reject: exceeds balance → InsufficientBalance()", "PASS"],
        ["T14 - changeVaultProof: rotate PROOF1 → PROOF2", "PASS"],
        ["T15 - old PROOF1 rejected after rotation", "PASS"],
        ["T16 - activityCount increments on each state change", "PASS"],
        ["T17 - qrypt 1.0 USDC with PROOF2 (replenish)", "PASS"],
        ["T18 - createAirVoucher signed off-chain (EIP-712)", "PASS"],
        ["T19 - claimAirVoucher: Wallet B receives 1.0 USDC", "PASS"],
        ["T20 - replay claimAirVoucher → VoucherAlreadyUsed()", "PASS"],
        ["T21 - expired voucher → VoucherExpired()", "PASS"],
        ["T22 - invalid signature → InvalidVoucherSig()", "PASS"],
        ["T23 - voucher nonce isolation: two vouchers, one consumed", "PASS"],
        ["T24 - unshieldToRailgun: burn qToken + deposit to Railgun pool", "PASS"],
        ["T25 - Railgun deposit event emitted with correct amount", "PASS"],
        ["T26 - qToken fully burned after Railgun route", "PASS"],
        ["T27 - unshieldToRailgun reject: zero railgunAddr → ZeroAddress()", "PASS"],
        ["T28 - unshieldToRailgun reject: wrong proof → InvalidProof()", "PASS"],
        ["T29 - createVault duplicate → VaultAlreadyExists()", "PASS"],
        ["T30 - initialize() on existing vault → AlreadyInitialized()", "PASS"],
        ["T31 - changeVaultProof zero hash → InvalidNewProof()", "PASS"],
        ["T32 - emergencyWithdraw before delay → EmergencyDelayNotMet()", "PASS"],
        ["T33 - getVault(owner) returns correct clone address", "PASS"],
        ["T34 - hasVault(owner) false for new address", "PASS"],
        ["T35 - hasVault(owner) true after createVault", "PASS"],
        ["T36 - vaultCreatedAt block stored correctly", "PASS"],
        ["T37 - lastActivityBlock updates on qrypt", "PASS"],
        ["T38 - lastActivityBlock updates on unqrypt", "PASS"],
        ["T39 - lastActivityBlock updates on claimAirVoucher", "PASS"],
        ["T40 - activityCount monotonic across mixed calls", "PASS"],
        ["T41 - multi-token: USDC + DAI shielded independently", "PASS"],
        ["T42 - multi-token: unqrypt one does not touch other", "PASS"],
        ["T43 - MINIMUM_SHIELD_AMOUNT constant = 1,000,000", "PASS"],
        ["T44 - getAllVaultAddresses() returns both Vault A + B", "PASS"],
        ["T45 - qToken non-transferable: transfer() reverts", "PASS"],
        ["T46 - qToken non-transferable: transferFrom() reverts", "PASS"],
        ["T47 - qToken balance zero for never-shielded token", "PASS"],
        ["T48 - Vault A qToken address ≠ Vault B qToken address", "PASS"],
        ["T49 - Vault A shield does not affect Vault B balance", "PASS"],
        ["T50 - unqrypt Vault A does not affect Vault B", "PASS"],
        ["T51 - full unqrypt leaves qToken balance = 0", "PASS"],
      ],
  ...
  ru: {
    model: {
      ...
      factorItems: [
        ...
        "OTP-цепочка proof: одноразовый vault proof передаётся как bytes32 в calldata. Контракт проверяет его по текущей голове OTP-цепочки (keccak256(proof) == chainHead), затем продвигает голову - каждая позиция одноразовая. Повтор потраченного proof завершается ошибкой \"Proof already used\".",
      ],
      pTwoFactorConclusion:
        "Злоумышленник, имеющий только приватный ключ, не может переместить токены, потому что передачи qToken всегда завершаются с ошибкой. Злоумышленник, имеющий только vault proof, не может вызвать функции хранилища, потому что onlyOwner блокирует любой адрес, не являющийся кошельком владельца. Даже если злоумышленник увидит proof в calldata, он уже потрачен: голова OTP-цепочки продвинулась, и потраченный proof больше никогда не будет принят.",
      ...
      pTwoFactor2:
        "OTP-цепочка и схема commit-reveal защищают от атак воспроизведения тремя механизмами:",
      replayItems: [
        "OTP-цепочка (основной): каждая позиция vault proof одноразовая. После использования голова цепочки продвигается на следующую позицию. Любая попытка повторно использовать потраченный proof немедленно отклоняется - использованная позиция недействительна навсегда.",
        'Nonce commit-reveal: каждый хэш коммита включает уникальный случайный nonce, выбранный браузером, поэтому даже одинаковые параметры передачи каждый раз дают разный хэш. Повторная попытка с тем же хэшем завершится ошибкой "Commit already used".',
        'Истечение коммита: коммит истекает через 600 секунд (10 минут) после отправки. Попытка раскрытия после этого окна завершится ошибкой "Commit expired".',
      ],
      ...
      attacks: [
        ...
        ["Повтор потраченного OTP proof", "Ошибка", "Голова цепочки продвинулась - потраченный proof отклоняется"],
        [
    ...
    nonCustodial: {
      ...
      custodyRows: [
        [
          ...
          "Адрес контракта PersonalVault (0xQryptSafeA)",
        ...
        [
          "Голова OTP-цепочки (хэш keccak256)",
          "Хранилище PersonalVault (публичное)",
          "Продвигается только при потреблении валидных OTP proof; пополняется через rechargeChain()",
        ],
      ],
    ...
    postQuantum: {
      ...
      algoRows: [
        ...
        [
          "keccak256 (SHA-3)",
          "Хранение головы OTP-цепочки (V6)",
          "256 бит",
      ...
      h2HowProtects: "Как keccak256 защищает OTP-цепочку",
      pHowProtects:
        "QryptSafe V6 использует OTP-цепочку (одноразовых proof) вместо статического хэша пароля. Контракт хранит chainHead - keccak256-хэш следующей валидной позиции proof. Сам proof никогда не хранится:",
      pPreimage:
        "Чтобы подделать валидный vault proof, злоумышленник должен найти прообраз текущего chainHead - обратить keccak256. Это атака на прообраз. Для 256-битного хэша классическая сложность составляет 2^256 операций. С алгоритмом Гровера это сокращается до 2^128 операций, что по-прежнему недостижимо для любого обозримого квантового компьютера. Для сравнения: 128-битная квантовая стойкость является порогом, принятым стандартами пост-квантовой криптографии NIST.",
      calloutNist:
      ...
      futureRows: [
        ...
        [
          ...
          "Голова OTP-цепочки (keccak256) по-прежнему блокирует доступ к хранилищу - поддельный ключ не может произвести следующий валидный OTP proof",
        ],
        [
          "Голова OTP-цепочки взломана квантовым путём",
          "Н/А",
          "128-битная квантовая стойкость; недостижимо с обозримым железом",
        ],
      ],
      pFutureConclusion:
        "В пост-квантовом мире, где подписи ECDSA взломаны, голова OTP-цепочки Qryptum (keccak256) становится последней линией обороны. В отличие от ECDSA, keccak256 не опирается на задачу дискретного логарифма на эллиптических кривых. Это односторонняя функция, остающаяся вычислительно сложной даже против квантовых противников.",
      ...
      honestItems: [
        ...
        "OTP proof в calldata: каждая позиция vault proof появляется в calldata транзакции во время Qrypt(), unQrypt() и revealTransfer(). Однако в V6 каждый proof одноразовый - к моменту появления on-chain он уже потреблён и голова цепочки продвинулась. Наблюдение исторического calldata даёт потраченные proof, которые контракт отклонит. Цепочку можно пополнить через rechargeChain() в любой момент.",
        "Инверсия головы цепочки: сохранённый chainHead не может быть использован для получения следующего валидного proof без обращения keccak256. Только законный владелец хранилища, знающий полное построение OTP-цепочки, может произвести следующий валидный proof.",
      ],
      pHonestConclusion:
        "Пост-квантовая устойчивость Qryptum конкретно связана с хранением головы OTP-цепочки (keccak256) и хэшем передачи commit-reveal. Они обеспечивают значимую защиту даже в сценариях, где ECDSA скомпрометирован. Структура OTP-цепочки добавляет дополнительное свойство: наблюдаемые в calldata proof уже потрачены и структурно бесполезны для злоумышленника.",
    },
    contractVerification: {
      ...
      h3V6Active: "v6: Активный (OTP-цепочка · air bags · 67/67 E2E · MIT верифицирован)",
      pV6: "Обновление безопасности от v5. OTP-цепочка заменяет статический bytes32 proofHash: каждый proof используется один раз и продвигает голову цепочки, делая атаки воспроизведения структурно невозможными. Изоляция air bags отделяет средства QryptAir от защищённого баланса. rechargeChain() позволяет пополнять OTP. 67/67 E2E-тестов проходят на Sepolia. MIT-верифицирован на Etherscan.",
      h3V5Active: "v5: Заменён v6 (bytes32 proofHash · 51/51 E2E · MIT верифицирован)",
      pV5: "Каноническое развёртывание v5 на Sepolia. bytes32 proofHash заменяет string vaultProof. unshieldToRailgun() для QryptShield, redeemVoucher() EIP-712 для QryptAir. 51/51 E2E-теста проходят. Все контракты MIT-верифицированы на Etherscan. Заменён v6 (обновление OTP-цепочки).",
      ...
      h2Mainnet: "Ethereum Mainnet",
      calloutMainnet:
        "QryptSafe запущен в основной сети Ethereum. Factory: 0xE3583f8cA00Edf89A00d9D8c46AE456487a4C56f. Implementation: 0x9E73602079fCbB918D22A7a8b57C2d99F5D701b4. Верификация на Etherscan выполняется.",
      ...
      coverageRows: [
        ["QryptSafeV1.test.js", "12", "EIP-1167 клон, фабрика Ownable, shield/unshield, commit-reveal"],
        ["QryptSafeV2.test.js", "23", "Исправление десятичных знаков, nonce commit, SafeERC20, непередаваемый qToken"],
        ["QryptSafeV3.test.js", "36", "Без admin-ключей, changeVaultProof, ECDSA metaTransfer, изоляция"],
        ["QryptSafeV4.test.js", "47", "Пользовательские ошибки (13), метаданные хранилища, частичный unshield, activityCount"],
        ["QryptSafeV5.test.js", "51", "qrypt/unqrypt, QryptAir EIP-712 ваучеры, Railgun ZK, 8 групп (T01-T51)"],
      ],
      coverageTotal: ["Итого", "169", "Все проходят"],
      h2E2EV6: "Результаты E2E-тестов (Sepolia, v6: 67/67)",
      e2eV6Rows: [
      ...
      h2E2EV5: "Результаты E2E-тестов (Sepolia, v5: 51/51)",
      e2eV5Rows: [
        ["T01 - Деплой фабрики и реализации QryptSafeV5 (EIP-1167)", "PASS"],
        ["T02 - Проверка префикса байткода EIP-1167 на фабрике", "PASS"],
        ["T03 - Подтверждение адреса реализации на фабрике", "PASS"],
        ["T04 - createVault: Wallet A → клон Vault A", "PASS"],
        ["T05 - createVault: Wallet B → клон Vault B", "PASS"],
        ["T06 - approve + qrypt 2.0 USDC (Wallet A)", "PASS"],
        ["T07 - qToken выпущен, баланс Vault A = 2.0", "PASS"],
        ["T08 - unqrypt 1.0 USDC (частичная сумма)", "PASS"],
        ["T09 - баланс qToken = 1.0 после частичного unqrypt", "PASS"],
        ["T10 - qrypt отклонён: неверный proof → InvalidProof()", "PASS"],
        ["T11 - qrypt отклонён: ниже минимума → InvalidAmount()", "PASS"],
        ["T12 - qrypt отклонён: не владелец → NotOwner()", "PASS"],
        ["T13 - unqrypt отклонён: превышение баланса → InsufficientBalance()", "PASS"],
        ["T14 - changeVaultProof: ротация PROOF1 → PROOF2", "PASS"],
        ["T15 - старый PROOF1 отклонён после ротации", "PASS"],
        ["T16 - activityCount увеличивается при каждом изменении состояния", "PASS"],
        ["T17 - qrypt 1.0 USDC с PROOF2 (пополнение)", "PASS"],
        ["T18 - createAirVoucher подписан офлайн (EIP-712)", "PASS"],
        ["T19 - claimAirVoucher: Wallet B получает 1.0 USDC", "PASS"],
        ["T20 - повтор claimAirVoucher → VoucherAlreadyUsed()", "PASS"],
        ["T21 - просроченный ваучер → VoucherExpired()", "PASS"],
        ["T22 - неверная подпись → InvalidVoucherSig()", "PASS"],
        ["T23 - изоляция nonce ваучера: два ваучера, один использован", "PASS"],
        ["T24 - unshieldToRailgun: сжигание qToken + депозит в пул Railgun", "PASS"],
        ["T25 - событие депозита Railgun с правильной суммой", "PASS"],
        ["T26 - qToken полностью сожжён после маршрута Railgun", "PASS"],
        ["T27 - unshieldToRailgun: нулевой railgunAddr → ZeroAddress()", "PASS"],
        ["T28 - unshieldToRailgun: неверный proof → InvalidProof()", "PASS"],
        ["T29 - дублирование хранилища → VaultAlreadyExists()", "PASS"],
        ["T30 - повторная initialize() → AlreadyInitialized()", "PASS"],
        ["T31 - нулевой хэш в changeVaultProof → InvalidNewProof()", "PASS"],
        ["T32 - emergencyWithdraw до задержки → EmergencyDelayNotMet()", "PASS"],
        ["T33 - getVault(owner) возвращает правильный адрес клона", "PASS"],
        ["T34 - hasVault(owner) false для нового адреса", "PASS"],
        ["T35 - hasVault(owner) true после createVault", "PASS"],
        ["T36 - vaultCreatedAt блок сохранён корректно", "PASS"],
        ["T37 - lastActivityBlock обновляется при qrypt", "PASS"],
        ["T38 - lastActivityBlock обновляется при unqrypt", "PASS"],
        ["T39 - lastActivityBlock обновляется при claimAirVoucher", "PASS"],
        ["T40 - activityCount монотонно растёт при смешанных вызовах", "PASS"],
        ["T41 - мультитокен: USDC + DAI защищены независимо", "PASS"],
        ["T42 - мультитокен: unqrypt одного не затрагивает другой", "PASS"],
        ["T43 - константа MINIMUM_SHIELD_AMOUNT = 1,000,000", "PASS"],
        ["T44 - getAllVaultAddresses() возвращает Vault A и Vault B", "PASS"],
        ["T45 - qToken непередаваем: transfer() откатывается", "PASS"],
        ["T46 - qToken непередаваем: transferFrom() откатывается", "PASS"],
        ["T47 - баланс qToken = 0 для никогда не защищённого токена", "PASS"],
        ["T48 - адрес qToken Vault A ≠ адресу qToken Vault B", "PASS"],
        ["T49 - shield Vault A не затрагивает баланс Vault B", "PASS"],
        ["T50 - unqrypt Vault A не затрагивает Vault B", "PASS"],
        ["T51 - полный unqrypt оставляет баланс qToken = 0", "PASS"],
      ],
  ...
  zh: {
    model: {
      ...
      factorItems: [
        ...
        "OTP 链 proof：一次性保险库密码以 bytes32 形式在 calldata 中传递。合约将其与当前 OTP 链头进行验证（keccak256(proof) == chainHead），然后推进链头 - 每个位置一次性使用。重放已消耗的 proof 将以 \"Proof already used\" 回滚。",
      ],
      pTwoFactorConclusion:
        "仅有私钥的攻击者无法移动代币，因为 qToken 转账始终回滚。仅有保险库密码的攻击者无法调用保险库函数，因为 onlyOwner 会阻止任何非所有者钱包的地址。即使攻击者在 calldata 中看到 proof，该 proof 也已被消耗：OTP 链头已推进，消耗的 proof 将永远不再被接受。",
      ...
      h2Replay: "防重放攻击",
      pTwoFactor2: "OTP 链和提交-揭示转账方案通过三种机制防止重放攻击：",
      replayItems: [
        "OTP 链（主要机制）：每个保险库密码位置一次性使用。消耗后，链头推进到下一个位置。任何重用已消耗 proof 的尝试都会立即回滚 - 消耗的位置永久失效。",
        '提交-揭示 nonce：每个提交哈希包含浏览器选择的唯一随机 nonce，因此即使相同的转账参数每次也会产生不同的哈希。使用相同哈希的第二次尝试将以 "Commit already used" 回滚。',
        '提交过期：提交在提交后 600 秒（10 分钟）到期。在此窗口后尝试揭示将以 "Commit expired" 回滚。',
      ],
      h2Attacks: "攻击场景",
      ...
      attacks: [
        ...
        ["重放已消耗的 OTP proof", "回滚", "链头已推进 - 消耗的 proof 被拒绝"],
    ...
    nonCustodial: {
      ...
      custodyRows: [
        [
          ...
          "PersonalVault 合约地址（0xQryptSafeA）",
          "仅保险库所有者通过保险库函数控制",
        ],
        [
          "qToken 余额",
        ...
        [
          "OTP 链头（keccak256 哈希）",
          "PersonalVault 存储（公开）",
          "仅通过消耗有效 OTP proof 推进；可通过 rechargeChain() 补充",
        ],
      ],
      h2NoAdmin: "无管理员访问",
      pNoAdmin1:
    ...
    postQuantum: {
      ...
      h2HowProtects: "keccak256 如何保护 OTP 链",
      pHowProtects:
        "QryptSafe V6 使用 OTP（一次性 proof）链代替静态密码哈希。合约存储 chainHead - 下一个有效 proof 位置的 keccak256 哈希。proof 本身从不存储：",
      pPreimage:
        "要伪造有效的保险库密码，攻击者必须找到当前 chainHead 的原像 - 反转 keccak256。这是一种原像攻击。对于 256 位哈希，经典复杂度为 2^256 次操作。在 Grover 算法下，这减少到 2^128 次操作，仍超出任何可预见量子计算机的能力范围。相比之下，128 位量子安全性是 NIST 后量子密码学标准采用的阈值。",
      calloutNist:
      ...
      futureRows: [
        ...
        [
          "攻击者伪造私钥签名",
          "完全访问代币",
          "OTP 链头（keccak256）仍阻止保险库访问 - 伪造的密钥无法产生下一个有效的 OTP proof",
        ],
        [
          "OTP 链头被量子计算破解",
          "不适用",
      ...
      pFutureConclusion:
        "在 ECDSA 签名可被破解的后量子世界中，Qryptum 的 OTP 链头（keccak256）成为最后一道防线。与 ECDSA 不同，keccak256 不依赖椭圆曲线离散对数问题。它是一个单向函数，即使面对量子对手也在计算上保持困难。",
      ...
      honestItems: [
        ...
        "calldata 中的 OTP proof：每个保险库密码位置在 Qrypt()、unQrypt() 和 revealTransfer() 期间出现在交易 calldata 中。然而在 V6 中，每个 proof 是一次性的 - 在链上可见时已被消耗，链头已推进。观察历史 calldata 只能获得已消耗的 proof，合约将拒绝这些 proof。链可随时通过 rechargeChain() 补充。",
        "链头反转：存储的 chainHead 无法用于在不反转 keccak256 的情况下推导下一个有效 proof。只有知道完整 OTP 链构造的合法保险库所有者才能产生下一个有效 proof。",
      ],
      pHonestConclusion:
        "Qryptum 的后量子弹性具体体现在 OTP 链头存储（keccak256）和提交-揭示转账哈希上。这些即使在 ECDSA 被攻破的场景下也能提供有意义的防御。OTP 链结构增加了额外属性：calldata 中可观察到的 proof 已被消耗，对攻击者结构上毫无用处。",
    },
    contractVerification: {
      title: "合约验证",
      intro:
      ...
      h3V6Active: "v6：活跃（OTP 链 · air bags · 67/67 E2E · MIT 已验证）",
      pV6: "v5 的安全升级。OTP 链替换静态 bytes32 proofHash：每个 proof 仅可使用一次并推进链头，从结构上使重放攻击不可能发生。Air bags 隔离将 QryptAir 资金与受保护余额分开。rechargeChain() 允许 OTP 补充。Sepolia 上 67/67 E2E 测试全部通过。工厂和实现已在 Etherscan 进行 MIT 验证。",
      h3V5Active: "v5：已被 v6 取代（bytes32 proofHash · 51/51 E2E · MIT 已验证）",
      pV5: "Sepolia 上的标准 v5 部署。bytes32 proofHash 替换 string vaultProof。unshieldToRailgun() 用于 QryptShield，redeemVoucher() EIP-712 用于 QryptAir。51/51 E2E 测试全部通过。所有合约已在 Etherscan 进行 MIT 验证。已被 v6（OTP 链升级）取代。",
      ...
      h2Mainnet: "以太坊主网",
      calloutMainnet:
        "QryptSafe 已在以太坊主网上线。Factory：0xE3583f8cA00Edf89A00d9D8c46AE456487a4C56f。Implementation：0x9E73602079fCbB918D22A7a8b57C2d99F5D701b4。Etherscan 验证进行中。",
      ...
      coverageRows: [
        ["QryptSafeV1.test.js", "12", "EIP-1167 克隆、Ownable 工厂、shield/unshield、commit-reveal"],
        ["QryptSafeV2.test.js", "23", "小数精度修复、nonce commit、SafeERC20、不可转让 qToken"],
        ["QryptSafeV3.test.js", "36", "无管理员密钥、changeVaultProof、ECDSA metaTransfer、隔离"],
        ["QryptSafeV4.test.js", "47", "自定义错误 (13)、保险库元数据、部分取消屏蔽、activityCount"],
        ["QryptSafeV5.test.js", "51", "qrypt/unqrypt、QryptAir EIP-712 凭证、Railgun ZK、8 组 (T01-T51)"],
      ],
      coverageTotal: ["总计", "169", "全部通过"],
      h2E2EV6: "E2E 测试结果（Sepolia，v6: 67/67）",
      e2eV6Rows: [
      ...
      h2E2EV5: "E2E 测试结果（Sepolia，v5: 51/51）",
      e2eV5Rows: [
        ["T01 - 部署 QryptSafeV5 工厂 + 实现（EIP-1167）", "PASS"],
        ["T02 - 验证工厂上的 EIP-1167 字节码前缀", "PASS"],
        ["T03 - 确认工厂存储的实现地址", "PASS"],
        ["T04 - createVault：Wallet A → Vault A 克隆", "PASS"],
        ["T05 - createVault：Wallet B → Vault B 克隆", "PASS"],
        ["T06 - approve + qrypt 2.0 USDC（Wallet A）", "PASS"],
        ["T07 - qToken 已铸造，Vault A 余额 = 2.0", "PASS"],
        ["T08 - unqrypt 1.0 USDC（部分金额）", "PASS"],
        ["T09 - 部分 unqrypt 后 qToken 余额 = 1.0", "PASS"],
        ["T10 - qrypt 拒绝：错误 proof → InvalidProof()", "PASS"],
        ["T11 - qrypt 拒绝：低于最小值 → InvalidAmount()", "PASS"],
        ["T12 - qrypt 拒绝：非所有者 → NotOwner()", "PASS"],
        ["T13 - unqrypt 拒绝：超出余额 → InsufficientBalance()", "PASS"],
        ["T14 - changeVaultProof：轮换 PROOF1 → PROOF2", "PASS"],
        ["T15 - 轮换后旧 PROOF1 被拒绝", "PASS"],
        ["T16 - activityCount 每次状态变更递增", "PASS"],
        ["T17 - 用 PROOF2 qrypt 1.0 USDC（补充）", "PASS"],
        ["T18 - createAirVoucher 离线签名（EIP-712）", "PASS"],
        ["T19 - claimAirVoucher：Wallet B 收到 1.0 USDC", "PASS"],
        ["T20 - 重放 claimAirVoucher → VoucherAlreadyUsed()", "PASS"],
        ["T21 - 过期凭证 → VoucherExpired()", "PASS"],
        ["T22 - 无效签名 → InvalidVoucherSig()", "PASS"],
        ["T23 - 凭证 nonce 隔离：两张凭证，一张已使用", "PASS"],
        ["T24 - unshieldToRailgun：销毁 qToken + 存入 Railgun 池", "PASS"],
        ["T25 - Railgun 存款事件以正确金额发出", "PASS"],
        ["T26 - Railgun 路由后 qToken 完全销毁", "PASS"],
        ["T27 - unshieldToRailgun 拒绝：零 railgunAddr → ZeroAddress()", "PASS"],
        ["T28 - unshieldToRailgun 拒绝：错误 proof → InvalidProof()", "PASS"],
        ["T29 - 重复创建保险库 → VaultAlreadyExists()", "PASS"],
        ["T30 - 对已有保险库 initialize() → AlreadyInitialized()", "PASS"],
        ["T31 - changeVaultProof 零哈希 → InvalidNewProof()", "PASS"],
        ["T32 - 延迟前 emergencyWithdraw → EmergencyDelayNotMet()", "PASS"],
        ["T33 - getVault(owner) 返回正确克隆地址", "PASS"],
        ["T34 - 新地址的 hasVault(owner) 为 false", "PASS"],
        ["T35 - createVault 后 hasVault(owner) 为 true", "PASS"],
        ["T36 - vaultCreatedAt 区块正确存储", "PASS"],
        ["T37 - qrypt 时 lastActivityBlock 更新", "PASS"],
        ["T38 - unqrypt 时 lastActivityBlock 更新", "PASS"],
        ["T39 - claimAirVoucher 时 lastActivityBlock 更新", "PASS"],
        ["T40 - activityCount 在混合调用中单调递增", "PASS"],
        ["T41 - 多代币：USDC + DAI 独立屏蔽", "PASS"],
        ["T42 - 多代币：unqrypt 一个不影响另一个", "PASS"],
        ["T43 - MINIMUM_SHIELD_AMOUNT 常量 = 1,000,000", "PASS"],
        ["T44 - getAllVaultAddresses() 返回 Vault A 和 Vault B", "PASS"],
        ["T45 - qToken 不可转让：transfer() 回滚", "PASS"],
        ["T46 - qToken 不可转让：transferFrom() 回滚", "PASS"],
        ["T47 - 从未屏蔽代币的 qToken 余额为 0", "PASS"],
        ["T48 - Vault A 的 qToken 地址 ≠ Vault B 的 qToken 地址", "PASS"],
        ["T49 - Vault A 的屏蔽不影响 Vault B 余额", "PASS"],
        ["T50 - Vault A 的 unqrypt 不影响 Vault B", "PASS"],
        ["T51 - 完整 unqrypt 后 qToken 余额 = 0", "PASS"],
      ],
