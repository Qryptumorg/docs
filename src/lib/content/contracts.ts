...
export const contractsContent: Record<"en" | "ru" | "zh", ContractsContent> = {
  en: {
    ...
    shieldFactory: {
      ...
      intro:
        "QryptSafe is the entry point for all Qrypt-Safe deployments. It uses the EIP-1167 minimal proxy pattern to deploy a unique PersonalVault for each wallet address. Version 6 is the current active deployment on Sepolia (OTP chain, air bags isolation, 50/50 E2E). v5, v3, and v2 are preserved below for historical reference.",
      calloutAddressV6: "Sepolia address (v6: active, OTP chain):",
      calloutV6Note: "v6 is the current canonical factory. OTP chain replaces static bytes32 proofHash. Air bags isolation for QryptAir. rechargeChain() for OTP top-up. 50/50 E2E tests pass. MIT-verified on Etherscan.",
  ...
  ru: {
    deployedAddresses: {
      ...
      h3V6Active: "v6: Активен (OTP-цепочка · air bags · 50/50 E2E · MIT-верифицирован)",
      pV6: "Обновление безопасности от v5. OTP-цепочка заменяет статический bytes32 proofHash: каждый proof используется один раз и продвигает голову цепочки, делая атаки воспроизведения структурно невозможными. Изоляция air bags отделяет средства QryptAir от защищённого баланса. rechargeChain() позволяет пополнять OTP. 50/50 E2E-тестов проходят на Sepolia. Фабрика и реализация MIT-верифицированы на Etherscan.",
  ...
  zh: {
    deployedAddresses: {
      ...
      h3V6Active: "v6：活跃（OTP 链 · air bags · 50/50 E2E · MIT 已验证）",
      pV6: "v5 的安全升级。OTP 链替换静态 bytes32 proofHash：每个 proof 仅可使用一次并推进链头，从结构上使重放攻击不可能发生。Air bags 隔离将 QryptAir 资金与受保护余额分开。rechargeChain() 允许 OTP 补充。Sepolia 上 50/50 E2E 测试全部通过。工厂和实现已在 Etherscan 进行 MIT 验证。",
    ...
    shieldFactory: {
      title: "QryptSafe（工厂）",
      intro:
        "QryptSafe 是所有 Qrypt-Safe 部署的入口点。它使用 EIP-1167 最小代理模式为每个钱包地址部署唯一的 PersonalVault。版本 6 是 Sepolia 上当前的活跃部署（OTP 链、air bags 隔离、50/50 E2E）。v5、v3 和 v2 保留在下方供历史参考。",
      calloutAddressV6: "Sepolia 地址（v6：活跃, OTP 链）：",
      calloutV6Note: "v6 是当前标准工厂。OTP 链替换静态 bytes32 proofHash。QryptAir 的 air bags 隔离。rechargeChain() 用于 OTP 补充。50/50 E2E 测试全部通过。已在 Etherscan 进行 MIT 验证。",
