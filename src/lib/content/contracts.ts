...
export const contractsContent: Record<"en" | "ru" | "zh", ContractsContent> = {
  en: {
    deployedAddresses: {
      ...
      pV5: "Canonical v5 deployment. bytes32 proofHash replaces string vaultProof, enabling EIP-712 struct-hash compatibility. unshieldToRailgun() added for QryptShield. redeemVoucher() added for QryptAir offline transfers. 51/51 E2E tests pass on Sepolia. MIT-verified on Etherscan. Superseded by v6 (OTP chain upgrade).",
    ...
    shieldFactory: {
      ...
      intro:
        "QryptSafe is the entry point for all Qrypt-Safe deployments. It uses the EIP-1167 minimal proxy pattern to deploy a unique PersonalVault for each wallet address. Version 6 is the current active deployment on Sepolia (OTP chain, air bags isolation, 67/67 E2E). v5, v3, and v2 are preserved below for historical reference.",
      calloutAddressV6: "Sepolia address (v6: active, OTP chain):",
      calloutV6Note: "v6 is the current canonical factory. OTP chain replaces static bytes32 proofHash. Air bags isolation for QryptAir. rechargeChain() for OTP top-up. 67/67 E2E tests pass. MIT-verified on Etherscan.",
      ...
      pCreateVault:
        "Deploys a new PersonalVault clone for msg.sender. Reverts if a vault already exists for this wallet. The initialChainHead must be keccak256(toUtf8Bytes(rawVaultProof)) computed off-chain in the browser - this seeds the OTP chain. The raw vault proof never reaches this function.",
  ...
  ru: {
    deployedAddresses: {
      ...
      h3V6Active: "v6: Активен (OTP-цепочка · air bags · 67/67 E2E · MIT-верифицирован)",
      pV6: "Обновление безопасности от v5. OTP-цепочка заменяет статический bytes32 proofHash: каждый proof используется один раз и продвигает голову цепочки, делая атаки воспроизведения структурно невозможными. Изоляция air bags отделяет средства QryptAir от защищённого баланса. rechargeChain() позволяет пополнять OTP. 67/67 E2E-тестов проходят на Sepolia. Фабрика и реализация MIT-верифицированы на Etherscan.",
      ...
      pV5: "Каноническое развёртывание v5. bytes32 proofHash заменяет string vaultProof, обеспечивая совместимость с EIP-712 struct-hash. Добавлен unshieldToRailgun() для QryptShield. Добавлен redeemVoucher() для QryptAir офлайн-переводов. 51/51 E2E-теста проходят на Sepolia. MIT-верифицирован на Etherscan. Заменён v6 (обновление OTP-цепочки).",
  ...
  zh: {
    deployedAddresses: {
      ...
      pV5: "标准 v5 部署。bytes32 proofHash 替换 string vaultProof，实现 EIP-712 struct-hash 兼容性。添加 unshieldToRailgun()（QryptShield）。添加 redeemVoucher()（QryptAir 离线转账）。Sepolia 上 51/51 E2E 测试全部通过。已在 Etherscan 进行 MIT 验证。已被 v6（OTP 链升级）取代。",
      ...
      calloutMainnet: "QryptSafe 已在以太坊主网上线。Factory：0xE3583f8cA00Edf89A00d9D8c46AE456487a4C56f。Implementation：0x9E73602079fCbB918D22A7a8b57C2d99F5D701b4。Etherscan 验证进行中。",
    ...
    shieldFactory: {
      title: "QryptSafe（工厂）",
      intro:
        "QryptSafe 是所有 Qrypt-Safe 部署的入口点。它使用 EIP-1167 最小代理模式为每个钱包地址部署唯一的 PersonalVault。版本 6 是 Sepolia 上当前的活跃部署（OTP 链、air bags 隔离、67/67 E2E）。v5、v3 和 v2 保留在下方供历史参考。",
      calloutAddressV6: "Sepolia 地址（v6：活跃, OTP 链）：",
      calloutV6Note: "v6 是当前标准工厂。OTP 链替换静态 bytes32 proofHash。QryptAir 的 air bags 隔离。rechargeChain() 用于 OTP 补充。67/67 E2E 测试全部通过。已在 Etherscan 进行 MIT 验证。",
